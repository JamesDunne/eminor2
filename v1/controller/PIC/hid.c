...
void USBCheckHIDRequest(void)
{
    ...
    switch(SetupPkt.bRequest)
    {
        case GET_REPORT:
            ...
            break;
        case SET_REPORT:
//            HIDSetReportHandler();      //2/8/05 JD this was not at the right location.
			pDst.bRam = (byte*)&USBEP0DataInBuffer;	//Set destination for report data.
			usb_stat.ctrl_trf_mem = _RAM;		//Set memory type 
			wCount._word = 0;					//reset counter 
			ctrl_trf_session_owner = MUID_HID;  //set owner to HID, service this when we have some data..	
            break;
        case GET_IDLE:
            ctrl_trf_session_owner = MUID_HID;
